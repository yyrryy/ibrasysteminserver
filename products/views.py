...
def updateproduct(request):
    ref=request.POST.get('ref').lower().strip().replace('§', '-')
    print('>> ref', ref)
...
def getclientprice(request):
    ...
    if target=='bl':
        ...
        if clientprice:
            print('>> assign client price in fc')
            price=clientprice.price
            remise=clientprice.remise
            return JsonResponse({
            'price':price,
...
def listreglementbl(request):
    thisyear=timezone.now().year
    reglements=PaymentClientbl.objects.all().order_by('-echance')[:50]
    print(date.today())
    ctx={
...
def loadjournalventefc(request):
    ...
    if term != '0':
        ...
        if year=='0':
            ...
            # means the year i not selected, so the records of the current year
            products = Outfacture.objects.filter(date__year=thisyear).filter(q_objects).order_by('-date')[start:end]
            ...
            products = Outfacture.objects.filter(date__year=year).filter(q_objects).order_by('-date')[start:end]
    ...
    if startdate != '0' and enddate != '0':
        startdate = datetime.strptime(startdate, '%Y-%m-%d')
        enddate = datetime.strptime(enddate, '%Y-%m-%d')
        bons=Outfacture.objects.filter(date__range=[startdate, enddate]).order_by('-date')[start:end]
        trs=''
        for i in bons:
            trs+=f'''
    ...
    if year=='0':
        # means the year i not selected, so the records of the current year
        products = Outfacture.objects.filter(date__year=thisyear).order_by('-date')[start:end]
        total=round(Outfacture.objects.filter(date__year=thisyear).aggregate(Sum('total'))['total__sum'] or 0, 2)
        totalqty=Outfacture.objects.filter(date__year=thisyear).aggregate(Sum('qty'))['qty__sum'] or 0
    else:
        products = Outfacture.objects.filter(date__year=year).order_by('-date')[start:end]
        total=round(Outfacture.objects.filter(date__year=year).aggregate(Sum('total'))['total__sum'] or 0, 2)
        totalqty=Outfacture.objects.filter(date__year=year).aggregate(Sum('qty'))['qty__sum'] or 0
    ...
    if comptable=='1':
        ...
        for i in bons:
            trs+=f'''
                ...
                '''
        return JsonResponse({
            'trs':render(request, 'fclist.html', {'bons':bons, 'loadmore':True}).content.decode('utf-8'),
            'has_more': len(bons) == per_page,
        })
    if term != '0':
        ...
        if startdate=='0' and enddate=='0':
            ...
            for i in bons:
                ...
                '''
            return JsonResponse({
                'trs':render(request, 'fclist.html', {'bons':bons, 'loadmore':True, 'term':term, 'year':year, 'startdate':startdate, 'enddate':enddate, 'comptable':comptable}).content.decode('utf-8'),
                'has_more': len(bons) == per_page,
                'total':total,
                'totaltva':totaltva,
            })
...
def searchforlistfc(request):
    ...
    for i in bons:
        trs+=f'''
            <tr class="ord {"text-danger" if i.ispaid else ''} fc-row" term={searchedterm} year={year} orderid="{i.id}" ondblclick="createtab('bonl{i.id}', 'Facture {i.facture_no}', '/products/facturedetails/{i.id}')">
                <td>{ i.facture_no }</td>
                <td>{ i.date.strftime("%d/%m/%Y")}</td>
                <td>{ i.total}</td>
                <td>{ i.tva}</td>
...
def yeardatabachat(request):
    ...
    if facture:
        ...
        bons=Itemsbysupplier.objects.filter(date__year=year).order_by('-date')[:50]
    trs=''
    for order in bons:
        trs+=f'''
...
def searchforjv(request):
    ...
    print('>> with year')
    searched=Livraisonitem.objects.filter(isfacture=False, date__year=year).filter(q_objects)
...
def updatenotification(request):
    ...
    # try:
    #     req.get('http://ibraparts.ma/products/updatenotification', {'notificationid':notificationid,'notification':notification})
...
def etude(request):
    ctx={
    'title':'Etude',
    'etudes':Etude.objects.all().order_by('-created_at')
    }
